...
"t (True, True, S, C) = (Some(C),S, Suc 0::nat)"|
...
proof
  ...
  proof-
    fix n
    have 1: "simple_path n = \<lparr>I_A=True,I_B=True, OUT= Some(1), STATE=INIT_NODE, COUNTER= Suc 0\<rparr>" using simple_path_def by simp
    (*show that the simple path has props of a path*)
    then have s_p_0: "simple_path 0 =  \<lparr>I_A=True,I_B=True, OUT= Some(1), STATE=INIT_NODE, COUNTER= Suc 0\<rparr>" using simple_path_def by blast
    then have cond1: "STATE(simple_path 0) = INIT_NODE" using s_p_0 by simp
    have RHS: "(OUT(simple_path n), STATE(simple_path (Suc n)), COUNTER(simple_path(Suc n))) = (Some(1),INIT_NODE, Suc 0)" using simple_path_def by simp
...
proof
  ...
  show " \<forall>n<n'.   I_B(p n) = True \<and> (\<forall>j. j \<in> {n<..<n'} \<longrightarrow> I_B (p j) = False) \<longrightarrow> COUNTER (p n') = count p n (n' - n)"
  proof(induction n')
    case 0
    then show ?case
      by presburger
  next
    ...
    proof
      fix n
      show "n < Suc n' \<longrightarrow> I_B(p n) = True \<and> (\<forall>j. j \<in> {n<..<Suc n'} \<longrightarrow> I_B (p j) = False) \<longrightarrow> COUNTER (p (Suc n')) = count p n (Suc n' - n)"
      proof
        assume "n < Suc n'"
        show "I_B(p n) = True \<and> (\<forall>j. j \<in> {n<..<Suc n'} \<longrightarrow> I_B (p j) = False) \<longrightarrow> COUNTER (p (Suc n')) = count p n (Suc n' - n)"
        proof
          assume " I_B(p n) = True \<and> (\<forall>j. j \<in> {n<..<Suc n'} \<longrightarrow> I_B (p j) = False)"
          then have b: "(\<forall>j. j \<in> {n<..<Suc n'} \<longrightarrow> I_B (p j) = False)" and c: " I_B(p n) = True" by auto
          then consider (a) "n<n'"|(b) "n > n'" |(c) "n = n'"by linarith
          then show "COUNTER (p (Suc n')) = count p n (Suc n' - n)"
          proof (cases)
            case a
            hence IH:  "COUNTER (p n') = count p n (n' - n)" using a b Suc c by simp
          ...
          next
            case c
            then have RHS: "count p n (Suc n' - n) = (if(I_A(p(n'))) then 1 else 0)" by simp
            have paths_1: "t(I_A(p(n')), I_B (p(n')), STATE(p(n')), COUNTER(p(n'))) = (OUT(p(n')), STATE(p(Suc n')), COUNTER(p(Suc n')))" using paths_def base by blast
            then consider (TT) "I_A(p(n')) =  True \<and> I_B (p(n')) = True "
              |(TF) "I_A(p(n')) =  True \<and> I_B (p(n')) = False"
              |(FF) "I_A(p(n')) =  False \<and> I_B (p(n')) = False"
            ...
            proof(cases)
              case TT
              then have "t(True, True, S1, COUNTER(p(n'))) =  (OUT(p(n')), STATE(p(Suc n')), COUNTER(p(Suc n')))"
                using all_p_s1 base paths_1 by auto
              then have "1 = COUNTER(p(Suc n'))" by simp
          
              then have "COUNTER(p (Suc n')) = count p n (Suc n' - n)" using RHS
              by (simp add: TT)
              then show ?thesis.
            next
              case TF
              then have "t(True, False, S1, COUNTER(p(n'))) =(OUT(p(n')), STATE(p(Suc n')), COUNTER(p(Suc n')))"
                using all_p_s1 base paths_1 by auto
              then have "COUNTER(p (Suc n')) = Suc(COUNTER(p(n')))" by simp
              then have "COUNTER(p (Suc n')) = 1"
                using TF \<open>I_B (p n) = True \<and> (\<forall>j. j \<in> {n<..<Suc n'} \<longrightarrow> I_B (p j) = False)\<close> c by auto
                then have "COUNTER(p (Suc n')) = count p n (Suc n' - n)" using RHS TF by simp
              then show ?thesis.
            next
              case FF
               then have "t(False, False, S1, COUNTER(p(n'))) =(OUT(p(n')), STATE(p(Suc n')), COUNTER(p(Suc n')))"
                using all_p_s1 base paths_1 by auto
              then have "COUNTER(p (Suc n')) = COUNTER(p(n'))" by simp
              then have "COUNTER(p (Suc n')) = 0"
                using FF \<open>I_B (p n) = True \<and> (\<forall>j. j \<in> {n<..<Suc n'} \<longrightarrow> I_B (p j) = False)\<close> c by auto
              then have "COUNTER(p (Suc n')) = count p n (Suc n' - n)" using RHS FF by simp
              thus ?thesis.
              next
              case FT
              then have "t(False, True, S1, COUNTER(p(n'))) =(OUT(p(n')), STATE(p(Suc n')), COUNTER(p(Suc n')))"
                using all_p_s1 base paths_1 by auto
              then have "COUNTER(p (Suc n')) = 0" by simp
              then have "COUNTER(p (Suc n')) = count p n (Suc n' - n)" using RHS FT by simp
              then show ?thesis.
            qed
          qed
      qed
